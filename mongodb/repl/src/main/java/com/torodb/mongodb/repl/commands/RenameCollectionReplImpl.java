...
public class RenameCollectionReplImpl extends ExclusiveReplCommandImpl<RenameCollectionArgument, Empty> {
    ...
    public Status<Empty> apply(Request req, Command<? super RenameCollectionArgument, ? super Empty> command,
        ...
        try {
            if (!replicationFilters.getCollectionPredicate().test(arg.getToDatabase(), arg.getToCollection())) {
                if (!replicationFilters.getCollectionPredicate().test(arg.getFromDatabase(), arg.getFromCollection())) {
