...
public class ConcurrentOplogBatchExecutor extends AnalyzedOplogBatchExecutor {
    ...
    private Stream<NamespaceJob> split(NamespaceJob namespaceJob) {
        ...
        for (AnalyzedOp job : jobs) {
            ...
            if (currentList.size() >= subBatchSize) {
                result.add(new NamespaceJob(namespaceJob.getDatabase(), namespaceJob.getCollection(), currentList));
                currentList = currentListFactory.get();
            }
    ...
    public static class ConcurrentOplogBatchExecutorMetrics extends AnalyzedOplogBatchExecutorMetrics {
        ...
        public ConcurrentOplogBatchExecutorMetrics(ToroMetricRegistry metricRegistry) {
            ...
            this.subBatchSizeMeter = metricRegistry.meter(NAME_FACTORY.createMetricName("subBatchSizeMeter"));
            this.subBatchSizeHistogram = metricRegistry.histogram(NAME_FACTORY.createMetricName("subBatchSizeHistogram"));
        }
    ...
    public static interface SubBatchHeuristic {
        /**
         * Given some metrics, this heuristic returns number of {@link AnalyzedOp ops} that each
         * sub batch should have.
         *
         * @param metrics
