...
public class ReplCoordinatorStateMachine extends IdleTorodbService {
    ...
    protected void shutDown() throws Exception {
        CompletableFuture.runAsync(this::_shutDown, executorService).join();
    ...
    CompletableFuture<StateChange> fromRecoveryToSecondary(OplogApplierService.Callback callback) {
        return CompletableFuture.supplyAsync(
                () -> _fromRecoveryToSecondary(callback),
                executorService
        );
    }
    ...
    CompletableFuture<StateChange> fromSecondaryToRecovery(RecoveryService.Callback callback) {
        return CompletableFuture.supplyAsync(
                () -> _fromSecondaryToRecovery(callback),
                executorService
        );
    }
    ...
    StateChange _fromRecoveryToSecondary(OplogApplierService.Callback callback) {
        StateChange result;
    ...
    StateChange _fromSecondaryToRecovery(RecoveryService.Callback callback) {
        StateChange result;
    ...
    /**
     * A wrapper method that, given the required information to transist to a
     * new state, checks preconditions and set several generic things.
     *
     * @param <C>
     * @param triedState         the state is trying to be started
     * @param callback           the argument (usually a callback) the new state
     *                           requires
    ...
    private StateChange _startSecondaryMode(
            OplogApplierService.Callback serviceCallback) {
        assert state == ReplCoordinatorState.SECONDARY;
        assert recoveryService == null || !recoveryService.isRunning();

        ReplCoordinatorState triedState = ReplCoordinatorState.SECONDARY;
    ...
    private StateChange _stopRecoveryMode() {
        assert state == ReplCoordinatorState.RECOVERY;
    ...
    private StateChange _stopSecondaryMode() {
        assert state == ReplCoordinatorState.SECONDARY;
    ...
    public static enum RejectionType {
        /**
         * The change that was tried to be applied is not valid on the current 
         * state.
         */
        ILLEGAL_CHANGE,
        /**
        ...
        NO_CHANGE,
        /**
         * It was impossible to start the new state because an unexpected error
         * happened.
         */
        UNEXPECTED_ERROR,
        /**
