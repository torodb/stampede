...
public interface StructureInterface {
    ...
    /**
     * Drops all torodb elements from the backend, including metatables and
     * their content.
     *
     * After calling this method, ToroDB cannot use the underlying backend
     * until metada is created again.
     * @param dsl
     */
    void dropAll(@Nonnull DSLContext dsl);

    /**
     * Drops all user elements from the backend, including metatables content
     * but not metatables.
     *
     * After calling this method, ToroDB sees the underlying backend as a fresh
     * system, simmilar to the one that is present the first time ToroDB starts.
     * @param dsl
     */
    ...
    default void checkMetaDataTables(@Nonnull DSLContext dsl) throws InvalidDatabaseException {
        ...
        if (!torodbSchema.isPresent()) {
            throw new InvalidDatabaseException("Schema '" +
                    TorodbSchema.IDENTIFIER + "' not found");
        }
