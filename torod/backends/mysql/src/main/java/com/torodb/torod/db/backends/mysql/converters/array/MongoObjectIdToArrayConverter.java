...
public class MongoObjectIdToArrayConverter implements ArrayConverter<JsonString, ScalarMongoObjectId> {
    ...
    public String toJsonLiteral(ScalarMongoObjectId value) {
        return "base64:type254:" + DatatypeConverter.printBase64Binary(value.getArrayValue());
    }

    @Override
    public ScalarMongoObjectId fromJsonValue(JsonString value) {
        if (!value.getString().startsWith("base64:type254:")) {
            throw new ToroImplementationException(
                    "A binary in escape format was expected, but " + value
                    + " was found"
            );
        }
        return new ByteArrayScalarMongoObjectId(DatatypeConverter.parseBase64Binary(value.getString().substring(15)));
    }
}
