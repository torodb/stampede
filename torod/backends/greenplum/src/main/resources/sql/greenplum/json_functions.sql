...
RETURNS text AS $$
    SELECT CASE WHEN $1 IS NULL THEN 'null' ELSE '"'||translate(regexp_replace($1,'("|\\|/|'||chr(8)||'|'||chr(12)||'|'||chr(10)||'|'||chr(13)||'|'||chr(9)||')','\\\1','g'),chr(8)||chr(12)||chr(10)||chr(13)||chr(9),'bfnrt')||'"' END;
$$ LANGUAGE SQL;
...
RETURNS text AS $$
    SELECT CASE WHEN $1 IS NULL THEN 'null' ELSE '"\\x'||encode($1,'hex')||'"' END;
$$ LANGUAGE SQL;

CREATE OR REPLACE FUNCTION torodb.to_json_value(json)
RETURNS text AS $$
    SELECT CASE WHEN $1 IS NULL THEN 'null' ELSE textin(json_out($1)) END;
$$ LANGUAGE SQL;
