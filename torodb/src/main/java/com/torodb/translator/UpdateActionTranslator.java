...
public class UpdateActionTranslator {
    ...
    private static boolean isReplaceUpdate(BSONObject updateObject) {
    ...
    private static UpdateAction translateReplaceObject(BSONObject updateObject) {
        return new SetDocumentUpdateAction(
                MongoValueConverter.translateObject(updateObject)
        );
    }

    private static UpdateAction translateComposedUpdate(BSONObject updateObject) {
    ...
    private static void translateSingleOperation(
        ...
        BSONObject castedValue = (BSONObject) value;
        switch (key) {
            case INCREMENT: {
    ...
    private static void translateMove(
        ...
        for (String key : argument.keySet()) {
            Collection<AttributeReference> attRefs
                    = parseAttributeReference(key);
            if (!(argument.get(key) instanceof String)) {
