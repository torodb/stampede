...
public class TimeBinding<T> implements Binding<Time, T> {
  ...
  public static <J, U extends KvValue<?>> DataTypeForKv<U> fromKvValue(Class<U> type,
      KvValueConverter<Time, J, U> converter) {
    return DataTypeForKv.from(new DefaultDataType<>(null, Time.class, "TIME"), converter,
        new TimeBinding<>(converter));
  }

  public static <U> DataType<U> fromType(Class<U> type, Converter<Time, U> converter) {
    return new DefaultDataType<>(null, Time.class, "TIME").asConvertedDataType(
        new TimeBinding<>(converter));
  }
