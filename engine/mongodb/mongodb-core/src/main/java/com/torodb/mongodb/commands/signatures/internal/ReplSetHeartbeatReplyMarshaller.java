...
public class ReplSetHeartbeatReplyMarshaller {
    ...
    private static final LongField CONFIG_VERSION_FIELD
            = new LongField("v");
    private static final TimestampField ELECTION_TIME_FIELD
            = new TimestampField("electionTime");
    private static final StringField ERR_MSG_FIELD
            = new StringField("errmsg");
    private static final IntField ERROR_CODE_FIELD = new IntField("code");
    private static final BooleanField HAS_DATA_FIELD
            = new BooleanField("hasData");
    private static final BooleanField HAS_STATE_DISAGREEMENT_FIELD
            = new BooleanField("stateDisagreement");
    private static final StringField HB_MESSAGE_FIELD
            = new StringField("hbmsg");
    private static final BooleanField IS_ELECTABLE_FIELD
            = new BooleanField("e");
    private static final BooleanField IS_REPL_SET_FIELD
            = new BooleanField("rs");
    private static final IntField MEMBER_STATE_FIELD
            = new IntField("state");
    private static final BooleanField MISMATCH_FIELD
            = new BooleanField("mismatch");
    ...
    private static final StringField REPL_SET_FIELD
            = new StringField("set");
    private static final HostAndPortField SYNC_SOURCE_FIELD
                ...
                default:
                    {
                        ...
                        throw new TypesMismatchException(ELECTION_TIME_FIELD.getFieldName(), "Date or Timestamp", foundType, "Expected \"" +
                                ELECTION_TIME_FIELD + "\" field in " +
                                "response to replSetHeartbeat command to " +
                                "have type Date or Timestamp, but found " +
                                "type " + foundType);
                    }
            }
        }
    ...
    private static boolean readHasData(BsonDocument bson) throws TypesMismatchException, NoSuchKeyException {
    ...
    @Nullable
    private static OpTime readNullableOpTime(BsonDocument bson, DocField field) throws TypesMismatchException, NoSuchKeyException {
    ...
    @Nullable
    private static MemberState readMemberState(BsonDocument bson) throws BadValueException, TypesMismatchException, NoSuchKeyException {
        MemberState state;
        if (!bson.containsKey(MEMBER_STATE_FIELD.getFieldName())) {
            ...
            try {
                ...
                throw new BadValueException("Value for \"" +
                        MEMBER_STATE_FIELD + "\" in response to " +
    ...
    private static long readConfigVersion(BsonDocument bson, Optional<OpTime> appliedOpTime) throws NoSuchKeyException, TypesMismatchException {
    ...
    @Nullable
    private static ReplicaSetConfig readConfig(BsonDocument bson) throws MongoException, FailedToParseException {
        BsonDocument uncastedConf;
        try {
            uncastedConf = BsonReaderTool.getDocument(bson, CONFIG_FIELD, null);
        } catch (TypesMismatchException ex) {
            throw ex.newWithMessage("Expected \"" + CONFIG_FIELD + "\" in " +
