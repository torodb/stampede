...
public class FindCommand extends AbstractNotAliasableCommand<FindArgument, FindResult> {
    ...
    public static class FindArgument {
        ...
        /**
         * Negative skip is illegal and a skip of zero received from the client is interpreted as
         * the absence of a skip value.
         */
        @Nonnegative
        private long skip;

        /**
         * Negative limit is illegal and a limit value of zero received from the client is
         * interpreted as the absence of a limit value.
         */
        @Nonnegative
        private long limit;
        ...
        private static void checkFieldType(Entry<?> entry, BsonType expectedType) throws FailedToParseException {
            if (entry.getValue().getType() != expectedType) {
                throw new FailedToParseException("Failed to parse " + entry.getKey() + ". It sould be of BSON type " + expectedType);
            }
            return ;
        }
        ...
        private static FindArgument unmarshall(BsonDocument requestDoc) throws FailedToParseException, TypesMismatchException, NoSuchKeyException, BadValueException {
        ...
        public static class Builder {
            ...
            private @Nonnull BsonDocument proj = DefaultBsonValues.EMPTY_DOC;
            private @Nonnull BsonDocument sort = DefaultBsonValues.EMPTY_DOC;
            /**
             * The hint provided, if any.  If the hint was by index key pattern, the value of '_hint' is
             * the key pattern hinted.  If the hint was by index name, the value of '_hint' is
             * {$hint: &lt;String&gt;}, where &lt;String&gt; is the index name hinted.
            ...
            public void validate() throws BadValueException {
                ...
                if ((limit > 0 || batchSize > 0) && ntoreturn > 0) {
                    throw new BadValueException("'limit' or 'batchSize' fields can not be set with 'ntoreturn' field.");
                }

                // Tailable cursors cannot have any sort other than {$natural: 1}.
                if (tailable) {
                    if (!sort.isEmpty() && !sort.equals(DefaultBsonValues.newDocument("$natural", DefaultBsonValues.INT32_ONE))) {
                        throw new BadValueException("cannot use tailable option with a sort other than {$natural: 1}");
                    }
                }
            }
        }
