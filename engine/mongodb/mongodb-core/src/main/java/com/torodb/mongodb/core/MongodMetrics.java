...
public class MongodMetrics {
	...
	public void createMetrics(Set<Command<?,?>> commands){
		for (Command<?,?> command : commands) {
			commandsTimerMap.put(command, registry.timer(factory.createMetricName(command.getCommandName()+"Timer")));
		}
	}
	
	public Timer getTimer(Command<?, ?> command){
		return commandsTimerMap.computeIfAbsent(command, c-> registry.timer(factory.createMetricName(c.getCommandName()+"Timer")));
	}
