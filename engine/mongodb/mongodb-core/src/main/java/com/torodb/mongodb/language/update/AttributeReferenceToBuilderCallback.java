...
class AttributeReferenceToBuilderCallback {
  ...
  /**
   * Descends through the value tree associated to the given builder as following keys and return
   * the {@linkplain BuilderCallback callback} associated with it.
   * <p>
   * @param <K>               the type of the keys of given builder
   * @param builder           the builder that will be the root of the given keys
   * @param keys              the keys path that will be followed
   * @param createIfNecessary if true, new child values will be created if there is no value with
   *                          the given keys in the given builder value tree. If false, null will be
   *                          returned in that case.
   * <p>
   * @throws UpdateException if keys is invalid in the given builder. That is: if there is an index
   *                         i where resolve(builder, keys.subList(0,i), createIfNecessary) is a
   *                         builder of type K1 and keys.get(i+1) is not of type K1 OR if
   *                         resolve(builder, keys.subList(0,i-1), createIfNecessary) has a scalar
   *                         value as child indexed with keys.get(i-1)
   */
  @Nullable
  ...
  private static <R, K> R resolve(
    ...
    UpdatedToroDocumentArrayBuilder nextArrayBuilder;
    UpdatedToroDocumentBuilder nextObjectBuilder;
    KvValue<?> nextValue;
