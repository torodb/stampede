/*
 * ToroDB
 * Copyright © 2014 8Kdata Technology (www.8kdata.com)
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program. If not, see <http://www.gnu.org/licenses/>.
 */

package com.torodb.mongodb.repl.filters;

import com.torodb.mongodb.commands.pojos.index.IndexOptions;
import com.torodb.mongodb.filters.DatabaseFilter;
import com.torodb.mongodb.filters.FilterResult;
import com.torodb.mongodb.filters.IndexFilter;
import com.torodb.mongodb.filters.NamespaceFilter;

/**
 * A simple {@link ReplicationFilters} defined by some predicates instead of functions that
 * return {@link FilterResult}.
 *
 * This replication filters are not as expressive as other, but are easier to configure with lambdas
 * and therefore are very useful on testings
 */
public abstract class SimpleReplicationFilters implements ReplicationFilters {

  public abstract boolean filterDatabase(String db);

  public abstract boolean filterNamespace(String db, String col);

  public abstract boolean filterIndex(IndexOptions idx);

  
  @Override
  public DatabaseFilter getDatabaseFilter() {
    return db -> {
      if (filterDatabase(db)) {
        return FilterResult.success();
      } else {
        return FilterResult.failure(this::getReason);
      }
    };
  }

  @Override
  public NamespaceFilter getNamespaceFilter() {
    return (db, col) -> {
      if (filterNamespace(db, col)) {
        return FilterResult.success();
      } else {
        return FilterResult.failure(this::getReason);
      }
    };
  }

  @Override
  public IndexFilter getIndexFilter() {
    return idx -> {
      if (filterIndex(idx)) {
        return FilterResult.success();
      } else {
        return FilterResult.failure(this::getReason);
      }
    };
  }

  public <E> String getReason(E e) {
    return e + " does not fulfill the filter";
  }

}
