...
import com.torodb.mongodb.commands.pojos.*;
...
class TopologyCoordinator {
    ...
    /**
     * Interval between the time the last heartbeat from a node was received successfully, or the
     * time when we gave up retrying, and when the next heartbeat should be sent to a target.
     */
    ...
    /**
     * the hostandport we are currently syncing from or {@link Optional#empty()} if no sync source
     * (we are cannot connect to anyone yet)
     */
    private @Nonnull Optional<HostAndPort> _syncSource;
    /**
     ...
     * @return the new sync source or {@link Optional#empty()} if we cannot calculate a new sync source yet
     */
    @Nonnull
     ...
     * @param now  the node will be removed from the black list if it is supposed to be
     *             unblacklisted by 'now.
     */
    ...
    ReplSetSyncFromReply executeReplSetSyncFrom(ErrorCode status, HostAndPort target, OpTime lastOpApplied)
            throws MongoException {
        if (status == ErrorCode.CALLBACK_CANCELED) {
            throw new ShutdownInProgressException("replication system is shutting down");
        }

        HostAndPort syncFromRequested = target;
    ...
    /**
     * Prepares a heartbeat request appropriate for sending to "target", assuming the current time
     * is "now".
     * <p>
     ...
     * @param ourSetName is used as the name for our replica set if the topology coordinator does
    ...
    RemoteCommandRequest<ReplSetHeartbeatArgument> prepareHeartbeatRequest(
        ...
        ReplSetHeartbeatArgument.Builder hbArgs = new ReplSetHeartbeatArgument.Builder(ReplSetProtocolVersion.V1)
                .setCheckEmpty(false);
        if (_rsConfig != null) {
        ...
        Duration timeoutPeriod = _rsConfig != null ?
     ...
     * <p>
     * Updates internal topology coordinator state, and returns instructions about what action to
     * take next.
     * <p>
     ...
     * {@link #updateConfig(com.eightkdata.mongowp.mongoserver.api.safe.library.v3m0.pojos.ReplicaSetConfig, java.time.Instant, com.eightkdata.mongowp.OpTime) updateConfig}
    ...
    HeartbeatResponseAction processHeartbeatResponse(
        ...
        boolean isUnauthorized = (hbResponse.getErrorCode() == ErrorCode.UNAUTHORIZED) ||
            (hbResponse.getErrorCode() == ErrorCode.AUTHENTICATION_FAILED);
        ...
        if (_rsConfig != null &&
            (hbStats.getNumFailuresSinceLastStart() <= MAX_HEARTBEAT_RETRIES) &&
            (alreadyElapsed.toMillis() < _rsConfig.getHeartbeatTimeoutPeriod())) {
            if (isUnauthorized) {
        ...
        if (!hbResponse.isOk()) {
            ...
            } else if (hbStats.getNumFailuresSinceLastStart() > MAX_HEARTBEAT_RETRIES ||
                       alreadyElapsed.toMillis() >= _rsConfig.getHeartbeatTimeoutPeriod()) {
     ...
     * {@link #processHeartbeatResponse(org.threeten.bp.Instant, org.threeten.bp.Duration, com.google.common.net.HostAndPort, com.eightkdata.mongowp.client.core.MongoConnection.RemoteCommandResponse, com.eightkdata.mongowp.OpTime) }.
     */
    private HeartbeatResponseAction updateHeartbeatDataImpl(int updatedConfigIndex, Instant now) {
        ...
        if (updatedConfigIndex == _currentPrimaryIndex) {
            MemberHeartbeatData updatedHBData = _hbdata.get(updatedConfigIndex);
    ...
    private static class PingStats {
        private @Nonnegative long count = 0;
        private @Nonnegative long value = UnsignedInteger.MAX_VALUE.longValue();
        ...
        /**
         * Gets the number of failures since {@link #start(org.threeten.bp.Instant) 'start'} was last called.
         * <p>
         * This value is incremented by calls to {@link #miss()}, cleared by calls to {@link #start()}
         * and set to {@link Integer#MAX_VALUE} by calls to {@link #hit(org.threeten.bp.Duration) hit()}.
         */
